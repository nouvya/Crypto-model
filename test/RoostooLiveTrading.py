"""
Live Trading Script for Roostoo (Real Orders)
---------------------------------------------------------------------
...
On Windows (cmd):
    set ROOSTOO_API_KEY=pRNOPgu5sN7Yk7fzJDrBUV9btWqpv0saO8SYL1uBM4nA2x0Mu4sxadWNLOMfMVtz
    set ROOSTOO_SECRET_KEY=RCFpa7PJGR1eT0c7TkVrRKA8NIhlFHL7W58xUexJx8cxl4HAQrQrMT8fZUzBdhBA
...
INITIAL_CAPITAL_PER_SYMBOL = 2500.0
THRESHOLD = 0.5
KELLY_B = 2.0
        ...
        if prob > self.threshold:
            if self.position == 0:
                ...
                if invest > 0:
                    ...
                    pair = f"{self.symbol}/USD"          # adjust format if needed
                    order_result = place_order_roostoo(
                        pair=pair,
                        side='BUY',
                        quantity=round(qty, 6),          # round according to exchange precision
                        order_type='MARKET'
                    )
